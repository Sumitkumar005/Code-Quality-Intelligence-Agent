"""
Main AI service for coordinating AI-powered features.
"""
...
class AIService:
    """
    Main AI service that coordinates various AI-powered features.
    """
    ...
    async def analyze_code_with_ai(
        ...
        try:
            ...
            embeddings = await self.embedding_service.generate_embeddings([code])

            # Use LLM for analysis
            analysis_prompt = self._create_analysis_prompt(code, language, analysis_type)
            analysis_result = await self.llm_service.generate_response(analysis_prompt)

            # Generate improvement suggestions
            improvement_prompt = self._create_improvement_prompt(code, language, analysis_result)
            improvements = await self.llm_service.generate_response(improvement_prompt)
    ...
    async def generate_code_suggestions(
        ...
        """
        try:
            suggestions = await self.code_generation_service.generate_suggestions(
    ...
    async def detect_code_smells(
        ...
        try:
            ...
            analysis = await self.llm_service.generate_response(smell_prompt)
